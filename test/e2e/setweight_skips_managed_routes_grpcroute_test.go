...
// TestSetWeightSkipsManagedRoutesGRPCRoute verifies that when setWeight is called,
// the weights of managed routes (header routes created by setHeaderRoute) are NOT modified.
// The test flow is:
// 1. Create GRPCRoute and Rollout
...
func TestSetWeightSkipsManagedRoutesGRPCRoute(t *testing.T) {
	feature := features.New("SetWeight skips managed routes for GRPCRoute").Setup(
		setupEnvironment,
	).Setup(
		setupSetWeightSkipsManagedRoutesGRPCRouteEnv,
	).Assess(
		"Verifying setWeight does not modify managed route weights",
		testSetWeightSkipsManagedRoutesGRPCRoute,
	).Teardown(
		teardownSetWeightSkipsManagedRoutesGRPCRouteEnv,
	).Feature()
	_ = global.Test(t, feature)
}

func setupSetWeightSkipsManagedRoutesGRPCRouteEnv(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	...
	firstGRPCRouteFile, err := os.Open(GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_PATH)
	if err != nil {
		logrus.Errorf("file %q openning was failed: %s", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_PATH, err)
		t.Error()
		return ctx
	}
	defer firstGRPCRouteFile.Close()
	logrus.Infof("file %q was opened", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_PATH)
	rolloutFile, err := os.Open(GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_ROLLOUT_PATH)
	if err != nil {
		logrus.Errorf("file %q openning was failed: %s", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	defer rolloutFile.Close()
	logrus.Infof("file %q was opened", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_ROLLOUT_PATH)
	err = decoder.Decode(firstGRPCRouteFile, &grpcRoute)
	if err != nil {
		logrus.Errorf("file %q decoding was failed: %s", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_PATH, err)
		t.Error()
		return ctx
	}
	logrus.Infof("file %q was decoded", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_PATH)
	err = decoder.Decode(rolloutFile, &rollout)
	if err != nil {
		logrus.Errorf("file %q decoding was failed: %s", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	logrus.Infof("file %q was decoded", GRPC_ROUTE_SETWEIGHT_SKIPS_MANAGED_ROLLOUT_PATH)
...
func testSetWeightSkipsManagedRoutesGRPCRoute(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	clusterResources := config.Client().Resources()
	...
	err = wait.For(
		waitCondition.ResourceMatch(
			resourcesMap[GRPC_ROUTE_KEY],
			getMatchManagedRouteWeightNilGRPCRouteFetcher(
				t,
	...
	logrus.Infof("grpcRoute %q was updated with nil weight for managed header route (setWeight skipped it correctly)", resourcesMap[GRPC_ROUTE_KEY].GetName())
	return ctx
}

func teardownSetWeightSkipsManagedRoutesGRPCRouteEnv(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	clusterResources := config.Client().Resources()
	resourcesMap, ok := ctx.Value(RESOURCES_MAP_KEY).(map[string]*unstructured.Unstructured)
	if !ok {
		logrus.Errorf("%q type assertion was failed", RESOURCES_MAP_KEY)
...
// getMatchManagedRouteWeightNilGRPCRouteFetcher returns a fetcher that checks:
...
func getMatchManagedRouteWeightNilGRPCRouteFetcher(t *testing.T, targetWeight int32, targetHeaderBasedRouteValue gatewayv1.GRPCHeaderMatch) func(k8s.Object) bool {
